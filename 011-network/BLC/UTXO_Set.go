...
func (utxoSet *UTXOSet) Update() {
	...
	WorkOutLoop:
		for index, out := range tx.Vouts {
			for _, in := range ins {
				if in.Vout == index && bytes.Compare(in.TxHash, tx.TxHash) == 0 && bytes.Compare(out.Ripemd160Hash, Ripemd160Hash(in.PublicKey)) == 0 {
	...
	err := utxoSet.Blockchain.DB.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			...
			for _, in := range ins {
				...
				for _, utxo := range txOutputs.UTXOS {
					if utxo.Index == in.Vout && bytes.Compare(utxo.Output.Ripemd160Hash, Ripemd160Hash(in.PublicKey)) == 0 {
						isNeedDelete = true
					} else {
						UTXOS = append(UTXOS, utxo)
					}
